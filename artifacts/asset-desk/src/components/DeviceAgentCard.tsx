...
export default function DeviceAgentCard({ assetId, assetTag }: Props) {
  ...
  // re-registering the asset, or changing inventory data. The command requires
  // a completed installation because only that installation can have a local
  // token file. An incomplete install must use the one-time install command.
  ...
  // Keep this as one physical line. Newline-based PowerShell snippets can
  // enter the interactive `>>` continuation prompt when pasted into an
  // elevated console, especially after a previous incomplete paste.
  const repairCmdWindows =
    `$ErrorActionPreference="Stop"; ` +
    `$principal=New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent()); ` +
    `if(-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)){throw "Open PowerShell with Run as administrator, then run this repair command again."}; ` +
    ...
    `if([string]::IsNullOrWhiteSpace($token)){throw "This is not a complete installation: no saved Windows agent key was found. Use Generate Key, then run the Windows install command instead of Repair."}; ` +
    ...
    `Invoke-WebRequest -Uri "${agentUrl}" -OutFile $tmp -UseBasicParsing -Headers @{"User-Agent"="miles-agent-bootstrap/Windows"}; ` +
    `if(-not (Test-Path $tmp) -or (Get-Item $tmp).Length -lt 1000){throw "The updated agent download was empty or incomplete."}; ` +
