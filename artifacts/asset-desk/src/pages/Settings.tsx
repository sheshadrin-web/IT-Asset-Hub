...
export default function Settings() {
  ...
  const [orgName,            setOrgName]            = useState("Miles Education Pvt Ltd");
  const [supportEmail,       setSupportEmail]       = useState("it.helpdesk@mileseducation.com");
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="support-email" type="email" value={supportEmail} onChange={e => setSupportEmail(e.target.value)} data-testid="input-support-email" />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          {[
            ...
            <div key={setting.id} className="flex items-center justify-between gap-4">
              ...
              <Switch id={setting.id} checked={setting.value} onCheckedChange={setting.onChange} data-testid={`switch-${setting.id}`} />
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <Switch id="two-factor" checked={twoFactor} onCheckedChange={setTwoFactor} data-testid="switch-two-factor" />
          </div>
          <Separator />
          <div className="space-y-1.5">
            <Label htmlFor="session-timeout">Session Timeout (minutes)</Label>
            <Input id="session-timeout" type="number" value={sessionTimeout} onChange={e => setSessionTimeout(e.target.value)} className="w-32" min="5" max="480" data-testid="input-session-timeout" />
