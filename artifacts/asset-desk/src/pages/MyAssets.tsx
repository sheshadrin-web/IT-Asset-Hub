...
import { Monitor, Smartphone, Calendar, MapPin, Package, ExternalLink } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { mockAssets, AssetStatus } from "@/data/mockData";
...
export default function MyAssets() {
  ...
  const myAssets = mockAssets.filter(
    (a) => a.assignedTo === currentUser?.name
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {myAssets.map((asset) => (
            <Card
              ...
              data-testid={`card-my-asset-${asset.assetId}`}
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span
                    className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium flex-shrink-0 ${assetStatusColors[asset.status]}`}
                  >
                    {asset.status}
                  </span>
                </div>
