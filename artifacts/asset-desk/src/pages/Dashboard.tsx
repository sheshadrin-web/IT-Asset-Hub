...
import { mockAssets, mockTickets, ROLE_LABELS } from "@/data/mockData";
...
function EndUserDashboard({ userName }: { userName: string }) {
  const myTickets = mockTickets.filter((t) => t.raisedBy === userName);
  const myAssets  = mockAssets.filter((a)  => a.assignedTo === userName);
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground">Asset ID</th>
                    <th className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground">Device</th>
                    <th className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground">Status</th>
                    <th className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground">Warranty</th>
                  </tr>
                </thead>
                <tbody>
                  {myAssets.map((a) => (
                    <tr key={a.assetId} className="border-b border-border last:border-0 hover:bg-muted/40">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium bg-blue-500/15 text-blue-600 border-blue-500/20">
                          {a.status}
                        </span>
                      </td>
                ...
                <tbody>
                  {recentTickets.map((t) => (
                    <tr key={t.ticketId} className="border-b border-border last:border-0 hover:bg-muted/40">
                      <td className="px-4 py-3">
                        <Link href={`/tickets/${t.ticketId}`} className="text-primary font-medium hover:underline">
                          {t.ticketId}
                        </Link>
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">{t.category}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium ${priorityColors[t.priority]}`}>
                          {t.priority}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium ${statusColors[t.status]}`}>
                          {t.status}
                        </span>
                      </td>
...
export default function Dashboard() {
  ...
  const statCards = [
    { label: "Total Assets",       value: mockAssets.length,                                                                                                          icon: Package,      color: "text-blue-500",    bg: "bg-blue-500/10",    href: "/assets" },
    { label: "Assigned Assets",    value: mockAssets.filter((a) => a.status === "Assigned").length,                                                                   icon: Monitor,      color: "text-indigo-500",  bg: "bg-indigo-500/10",  href: "/assets" },
    { label: "Available Assets",   value: mockAssets.filter((a) => a.status === "Available").length,                                                                  icon: CheckCircle,  color: "text-emerald-500", bg: "bg-emerald-500/10", href: "/assets" },
    { label: "Under Repair",       value: mockAssets.filter((a) => a.status === "Under Repair").length,                                                               icon: Wrench,       color: "text-amber-500",   bg: "bg-amber-500/10",   href: "/assets" },
    { label: "Open Tickets",       value: mockTickets.filter((t) => !["Resolved", "Closed", "Rejected"].includes(t.status)).length,                                   icon: Ticket,       color: "text-blue-500",    bg: "bg-blue-500/10",    href: "/tickets" },
    { label: "Critical Tickets",   value: mockTickets.filter((t) => t.priority === "Critical" && !["Resolved", "Closed", "Rejected"].includes(t.status)).length,     icon: AlertTriangle,color: "text-red-500",     bg: "bg-red-500/10",     href: "/tickets" },
    { label: "Resolved This Month",value: mockTickets.filter((t) => t.status === "Resolved").length,                                                                  icon: TrendingUp,   color: "text-emerald-500", bg: "bg-emerald-500/10", href: "/tickets" },
  ];
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={assetStatusData.filter((d) => d.value > 0)} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value">
                  {assetStatusData.filter((d) => d.value > 0).map((entry) => (
