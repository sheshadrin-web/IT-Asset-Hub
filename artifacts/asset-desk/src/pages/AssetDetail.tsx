...
import { mockTickets, mockUsers, AssetStatus } from "@/data/mockData";
...
export default function AssetDetail() {
  ...
  const handleAssignConfirm = () => {
    ...
    assignAsset(asset.assetId, selectedUser.name, selectedUser.department);
    toast({ title: "Asset assigned", description: `Assigned to ${selectedUser.name}` });
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isAdmin && asset.status !== "Assigned" && asset.status !== "Retired" && (
            <Button
              ...
              size="sm"
              className="gap-2"
              onClick={() => { setAssignDialogOpen(true); setAssignUser(""); }}
          ...
          {isAdmin && asset.status !== "Under Repair" && asset.status !== "Retired" && (
            <Button
              ...
              onClick={() => { updateStatus(asset.assetId, "Under Repair"); toast({ title: "Marked as Under Repair" }); }}
              data-testid="button-mark-repair"
            >
          ...
          {isAdmin && asset.status !== "Retired" && (
            <Button
              ...
              onClick={() => { updateStatus(asset.assetId, "Retired"); toast({ title: "Asset retired" }); }}
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold">
                Related Tickets{" "}
                <span className="text-muted-foreground font-normal">({relatedTickets.length})</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {relatedTickets.length === 0 ? (
                <p className="px-4 py-8 text-center text-sm text-muted-foreground">
                  No tickets raised for this asset.
                </p>
              ) : (
                    ...
                    <tbody>
                      {relatedTickets.map((t) => (
                        <tr key={t.ticketId} className="border-b border-border last:border-0 hover:bg-muted/30">
                          ...
                          <td className="px-4 py-3">
                            <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium", PRIORITY_COLORS[t.priority])}>{t.priority}</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium", TICKET_STATUS_COLORS[t.status])}>{t.status}</span>
                          </td>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{asset.warrantyEndDate}</span>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <Dialog open={assignDialogOpen} onOpenChange={(v) => !v && setAssignDialogOpen(false)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Assign {asset.assetId}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div>
              ...
              <Select value={assignUser} onValueChange={setAssignUser}>
                <SelectTrigger data-testid="select-assign-user-detail">
                  <SelectValue placeholder="Choose a user…" />
                </SelectTrigger>
                <SelectContent>
                  {activeUsers.map((u) => (
                    <SelectItem key={u.userId} value={u.userId}>
                      <span>{u.name} <span className="text-muted-foreground text-xs">— {u.department}</span></span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAssignConfirm} disabled={!assignUser} data-testid="button-confirm-assign-detail">
              Assign
            </Button>
          </DialogFooter>
        </DialogContent>
